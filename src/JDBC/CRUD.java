...
//	 NAVER 회원정보 가입하기 CREATE
	public void insert() {
		// 회원정보 DB에 insert하기!
		// n_insert() 메소드 생성 후 JdbcMain 클래스에서 실행하기
		...
		try {
			...
			String sql = "INSERT INTO NAVER VALUES('아이디5','998877','또치', 36 ,'여','Ehcldla@daum.net','01014701470')";
...
//	 회원정보 조회 SELECT
	public void select() {
		...
		try {
			...
			while(rs.next()) {
				// rs.next() : 데이터 타입은 boolean, 레코드가 존재할 경우 true, 더 이상 존재하지 않을 경우 false, 반복문 종료
...
//	 회원정보 수정 UPDATE
	public void update() {
		...
		try {	// 문제가 없으면 try문 실행, catch문 실행x
			...
			String sql = "UPDATE NAVER SET NAGE=30 WHERE NID='아이디5'";
...
//	 회원정보 삭제 DELETE
	public void delete() {
		...
		// [4] 확인단계 : while 또는 if
		// [5] 해제단계 : close (나중에 선언한걸 먼저 종료, 먼저 선언한걸 나중에 종료)
		
		try {
			con = DBConnection.DBconnect();
			stmt = con.createStatement();
			
			String sql = "DELETE FROM NAVER WHERE NID='아이디5'";
